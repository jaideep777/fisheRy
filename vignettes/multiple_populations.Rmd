...
### Simulate 1 population

```{r}
nsteps = 50
pop$verbose = T

res = sim$simulate_multi_2d(pop, 5.61, 45, 0.3, nsteps, 1.93e3, F)

as.data.frame(res[,1,1,1,]) %>% 
  setNames(pop$colnames) %>% 
  mutate(t = 1:n()) %>% 
  select(t, ssb, yield, employment.sea, nsuperfish, nfish_ra) %>% 
  pivot_longer(-t) %>% 
