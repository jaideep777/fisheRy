...
bool Stock::isFishable(const Fish &f){
...
/// \f]
double Stock::fishableBiomass(){
    return 
    std::accumulate(fishes.begin(), fishes.end(), 0.0, 
        [this](double sum, const Fish& f) { return sum + ((f.isAlive && isFishable(f)) ? par.n * f.weight : 0); }
    );
}
...
double Stock::calcAbundanceAtAge(int age){
    return 
    std::accumulate(fishes.begin(), fishes.end(), 0.0, 
        [](double sum, const Fish& f) { return sum + ((f.isAlive && f.age == age) ? par.n : 0); }
    );
} 


// vector<double> Stock::calcSB(){
// 	double tsb = 0, ssb = 0;
// 	for (auto& f : fishes){
